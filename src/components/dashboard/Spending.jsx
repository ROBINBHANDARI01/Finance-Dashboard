...
export default function Spending() {
  ...
  return (
    <div className="md:mt-0 p-5  bg-(--surface) border border-(--border) rounded-2xl">
      ...
      <div className="flex flex-col xs:flex-row  md:flex-row items-center ">

        {/* Chart + Center Text */}
        <div className="relative w-52 h-55 shrink-0">
  ...
  {spendingBreakdown.map((item, index) => (
    <div key={index} className="flex items-center justify-between">
      ...
      <span className="text-xs font-medium w-8 text-right shrink-0">{item.percent}%</span>
    </div>
  ))}
</div>
